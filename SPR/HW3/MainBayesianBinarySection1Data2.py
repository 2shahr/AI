...
# Load data
Raw_data = pd.read_csv(r'BC-Train2.csv')
x_trn = Raw_data.values[:, 0:-1]
y_trn = Raw_data.values[:, -1]

Raw_data = pd.read_csv(r'BC-Test2.csv')
...
ax.plot3D(x_trn[np.where(y_trn == 1)[0], 0],
          x_trn[np.where(y_trn == 1)[0], 1], F1, '.y')
